...
class EarthNetScore(Metric):
    ...
    def forward(self, *args, **kwargs):
        ...
        if self.compute_on_step:
            ...
            for el in self.data:
                el.update(
                    {
                        "ENS": self.__harmonic_mean(
                            [el["MAD"], el["OLS"], el["EMD"], el["SSIM"]]
                        )
                    }
                )
    ...
    def update(self, preds, targs):
        ...
        masks = (
            copy.deepcopy(targs["dynamic_mask"][0])
            .cpu()
            .numpy()[:, -preds.shape[1] :, ...]
        )
        cubenames = copy.deepcopy(targs["cubename"])
        targs = (
            copy.deepcopy(targs["dynamic"][0]).cpu().numpy()[:, -preds.shape[1] :, ...]
        )

        ndvi_preds = (
            (preds[:, :, 3, ...] - preds[:, :, 2, ...])
            / (preds[:, :, 3, ...] + preds[:, :, 2, ...] + 1e-6)
        )[:, :, np.newaxis, ...]
        ndvi_targs = (
            (targs[:, :, 3, ...] - targs[:, :, 2, ...])
            / (targs[:, :, 3, ...] + targs[:, :, 2, ...] + 1e-6)
        )[:, :, np.newaxis, ...]
        ...
        ens = np.array(
            [
                self.__harmonic_mean(
                    [sample["MAD"], sample["OLS"], sample["EMD"], sample["SSIM"]]
                )
